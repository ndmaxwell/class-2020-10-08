...
```{r sc1b}
draws %>% 
  filter(state %in% c("Ohio", "Wisconsin")) %>% 
  ggplot(aes(x = trump_share, fill = state)) +
    ...
    labs(title = "Estimated Trump Vote Share in Two States", 
         x = "Modeled Trump Share", y = "Density")
```
...
elections %>% 
  ...
  mutate(both = ifelse(pa_win & oh_win, TRUE, FALSE)) %>% 
  summarize(pa_avg = mean(pa_win), oh_avg = mean(oh_win), 
            both_avg = mean(both)) 
...
elections %>% 
  ...
  mutate(oh_win = map_lgl(simulations, ~ .$Ohio > 50)) %>%
  group_by(oh_win) %>% 
  summarize(pa_win_avg = mean(pa_win), .groups = "drop")
...
```{r}
elections_100 <- tibble(simulations = map(rep(1, 100), simulate_election))
